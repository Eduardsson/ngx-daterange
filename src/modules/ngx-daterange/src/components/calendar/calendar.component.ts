...
export class CalendarComponent implements OnChanges {
  ...
  getWeeksRange(weeks: number[]): DateRange[] {
    ...
    for (let i = 0; i < weeks.length; i++) {
      ...
      let firstWeekDay = moment([this.year, this.month])
        .week(week)
        .startOf("week");
      let lastWeekDay = moment([this.year, this.month])
        .week(week)
        .endOf("week");
      ...
      weeksRange.push(
        range(
          firstWeekDay.week(week).startOf("week"),
          lastWeekDay.week(week).endOf("week")
        )
      );
    }
  ...
  createCalendarGridData(): void {
    ...
    const endDay = moment([this.year, this.month])
      .endOf("month")
      .endOf("week");
